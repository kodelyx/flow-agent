...
window.fetch = async function (...args) {
  try {
    ...
    // ─── SNIFF ALL outgoing aisandbox requests ────────────
    if (url.includes('aisandbox-pa.googleapis.com')) {
      let bodyText = '';
      if (args[1]?.body) {
        bodyText = typeof args[1].body === 'string' ? args[1].body : JSON.stringify(args[1].body);
      }
      // Use postMessage to cross MAIN→ISOLATED world boundary
      window.postMessage({
        type: '__FLOWKIT_SNIFF__',
        url, body: bodyText, method: args[1]?.method || 'POST',
      }, '*');
    }
  } catch {}
